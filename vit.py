...
class SimpleViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels = 3, dim_head = 64, additional_patch_size = None, hybrid_mode: bool = False):
        super().__init__()
        ...
        if additional_patch_size:
            ...
            self.to_patch_embedding_b, self.pos_embedding_b = self._make_embeddings(
                patch_height_b, patch_width_b, patch_dim_b, dim, image_height, image_width
            )

        patch_dim = channels * patch_height * patch_width

        self.to_patch_embedding, self.pos_embedding = self._make_embeddings(
            patch_height, patch_width, patch_dim, dim, image_height, image_width
        )
    ...
    def forward(self, img, recurse=True):
        if self.hybrid_mode and recurse:
            a = self(img, recurse=False)
            self.toggle_embeddings()
            b = self(img, recurse=False)
            self.toggle_embeddings()
